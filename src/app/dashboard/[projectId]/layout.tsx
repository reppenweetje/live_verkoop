...
export default function ProjectLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen" style={{ background: "#0f0f70" }}>
      {/* Sidebar */}
      <aside
        className="sticky top-0 h-screen flex flex-col flex-shrink-0 overflow-hidden transition-all duration-300"
        style={{
      ...
      >
        ...
        <button
          ...
          className="absolute top-4 right-3 z-10 flex items-center justify-center w-7 h-7 rounded-lg transition-colors hover:bg-white/10"
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          >
            {collapsed ? (
              /* Ingeklapt: alleen logo/icoon */
              <div className="flex justify-center">
                {logo ? (
                  <Image src={logo} alt={projectDisplayName} width={32} height={32} className="w-8 h-8 object-contain" />
                ) : (
                  <span className="text-xs font-black text-white">{projectDisplayName.charAt(0)}</span>
                )}
              </div>
            ) : (
              /* Uitgeklapt: volledige header */
              <div>
                <p
          ...
          {/* Nav */}
          <nav className="flex-1 overflow-y-auto py-4 space-y-1" style={{ padding: collapsed ? "16px 8px" : "16px 24px" }}>
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={cn("nav-link", isActive && "nav-link-active", collapsed && "justify-center px-0")}
                  style={{
                ...
                >
                  ...
                  {/* Actief-dot bij ingeklapt */}
                  {collapsed && isActive && (
                    <span className="absolute right-1 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full" style={{ background: "#edff00" }} />
                  )}
                </Link>
              );
            })}
          ...
          {/* Footer */}
          {!collapsed && (
            <div style={{ borderTop: "1px solid rgba(237,255,0,0.08)", padding: "24px", flexShrink: 0 }}>
          ...
          {collapsed && (
            <div style={{ borderTop: "1px solid rgba(237,255,0,0.08)", padding: "16px 8px", flexShrink: 0 }}>
              <Link
                href="/dashboard"
